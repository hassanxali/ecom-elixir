defmodule EcomWeb.CheckoutLive do
  use EcomWeb, :live_view

  def mount(_params, _session, socket) do
    {:ok, assign(socket, cart_items: [], total_price: 0)}
  end
  ...
  def handle_event("place_order", %{}, socket) do
    IO.puts("Placing order")
    ids = Enum.into(socket.assigns.cart_items, [], fn item -> item.id end)

    IO.inspect(ids)

    {:ok, _} =
      Ecom.Orders.create_order(%{
        total_price: socket.assigns.total_price + 10,
        status: "Confirmed",
        user_id: 1,
        product_ids: ids
      })
  ...
  def render(assigns) do
    ...
    ~H"""
    <div phx-hook="restore" id="restore_checkout" class="flex flex-col items-center border-b bg-white py-4 sm:flex-row sm:px-10 lg:px-20 xl:px-32">
    ...
    <form class="mt-5 grid gap-6">
      <div class="relative">
        ...
        <label class="peer-checked:border-2 peer-checked:border-gray-700 peer-checked:bg-gray-50 flex cursor-pointer select-none rounded-lg border border-gray-300 p-4" for="radio_1">
          ...
          <div class="ml-5">
            <span class="mt-2 font-semibold">COD</span>
    ...
    <div class="">
      ...
      <div class="mt-6 flex items-center justify-between">
        ...
        <p class="text-2xl font-semibold text-gray-900">$<%= @total_price + 10 %></p>
      </div>
    </div>
    <button phx-click="place_order" phx-hook="store" id="place" class="mt-4 mb-8 w-full rounded-md bg-gray-900 px-6 py-3 font-medium text-white">Place Order</button>
    </div>
    </div>
